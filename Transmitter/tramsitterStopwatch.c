// WIRING:
//   P1.4 -> 150ohm -> SFH4546 anode -> cathode -> GND   (IR TX)
...
//   LCD: P1.0=D7 P1.1=D6 P1.2=D5 P1.3=D4 P1.7=RS P2.0=E
...
#define CMD_PATH_4      0x13  // NEW: Path 4
...
// Program Mode State
// ============================================================
unsigned char xdata program_path[8];  // CHANGED: moved to XDATA
unsigned char program_step = 0;       // Keep in DATA (single byte)

// ============================================================
// Stopwatch State (for automode)
// ============================================================
...
void Program_Mode(void);  // NEW: Program Mode function
const char *Direction_Label(unsigned char dir);  // NEW: Direction label helper
...
const char *Cmd_Label(unsigned char cmd)
{
    switch (cmd) {
        ...
        case CMD_PATH_4:     return "PATH  4  ";  // NEW
...
// NEW: Direction label for program mode
...
void Send_Joystick_Proportional(void)
{
    ...
    char xdata disp[17];  // CHANGED: moved to XDATA
    unsigned char percent;

    urx = ADC_at_Pin(JOY_URX_CH);
    ury = ADC_at_Pin(JOY_URY_CH);

    // Joystick scaling logic (kept same as your original)
    ...
    // Fixed Display: "L100 R100 B:95%" 
    // (Compact format to fit 16 chars)
    disp[0]='L';
...
unsigned char Password_Entry_Screen(char *line1_msg)
{
    ...
    char xdata line2[17];  // CHANGED: moved to XDATA
    char pass_str[5];      // Small, can stay in DATA
...
// ============================================================
// Timer2 for Stopwatch (1 Hz interrupt)
// ============================================================
void Init_Timer2_1Hz(void)
{
    // Timer2 in 16-bit auto-reload mode
    // SYSCLK = 72 MHz
    // We want 100 Hz (10 ms) interrupt
    // Prescaler = SYSCLK/12 = 6 MHz
    // For 100 Hz: Reload = 65536 - 60000 = 5536
    
    TMR2CN0 = 0x00;     // Stop Timer2, clear overflow flag
    CKCON0 &= ~0x30;    // Timer2 uses SYSCLK/12 = 6 MHz
	TMR2RL = 5536;    // Reload value for 100 Hz (10 ms)
    TMR2 = TMR2RL;      // Initialize Timer2
...
// ============================================================
// NEW: Program Mode Function - FIXED VERSION
// ============================================================
void Program_Mode(void)
{
    ...
    unsigned char i;
    unsigned char rx_ack;  // NEW: Store acknowledgment
    ...
    // DEBUG: Show if STM32 acknowledged
        ...
        else if (BTN3 == 0)
        {
            ...
            if (BTN3 == 0)
            {
                ...
                if (btn3_held >= BTN3_LONG_MS)
                {
                    ...
                    IR_TX = 0;
                    waitms(100);  // INCREASED: Give STM32 time to beep
                ...
                {
                    // ===== SHORT PRESS: Confirm current option and move to next step =====
                    program_path[program_step] = current_option;
...
void main(void)
{
    ...
    // ============================================================
    // PASSWORD SETUP SCREEN (only happens once after reset)
    // ============================================================
    ...
    {
        // ============================================================
        // Update stopwatch if in automode
        // ============================================================
        ...
        // ============================================================
        // LOCKED MODE - Password entry required
        // ============================================================
        if (is_locked)
        {
        ...
        if (auto_mode == 1)
        {
            // In AUTO MODE: BTN3 = PAUSE (existing behavior)
            if (BTN3 == 0)
            {
        ...
        // --------------------------------------------------------
        // BTN1: Horn (short press) OR Program Mode (long press in manual mode)
        // --------------------------------------------------------
        if (BTN1 == 0)
        {
            waitms(20);  // Debounce
            if (BTN1 == 0)
            {
                ...
                if (auto_mode == 0)
                {
                    ...
                    {
                        // LONG PRESS in MANUAL: Enter Program Mode
                        while (BTN1 == 0);  // Wait for release
                        Program_Mode();
                ...
                {
                    // In AUTO mode: Always horn (no long press check)
                    rx = IR_LoopbackPacket(CMD_HORN);
                    Show_Command(CMD_HORN);
                    while (BTN1 == 0);  // Wait for release
                    waitms(200);
        ...
        // --------------------------------------------------------
        // BTN2: short = cycle path (Manual only) OR pause stopwatch (Auto only)
        //       long = auto toggle
        // --------------------------------------------------------
        else if (BTN2 == 0)
        {
            ...
            if (BTN2 == 0)
            {
                ...
                {
                    // LONG PRESS: Toggle between Auto and Manual
                    auto_mode = !auto_mode;
                ...
                else
                {
                    // SHORT PRESS
                    if (auto_mode == 1) {
                        // In AUTO MODE: Pause/Resume stopwatch
                        ...
                        current_path++;
                        if (current_path > 4) current_path = 1;  // NOW cycles 1-4
        ...
        // --------------------------------------------------------
        // Joystick direction (only update display if NOT in automode)
        // --------------------------------------------------------
        else
        {
            if (auto_mode) {
