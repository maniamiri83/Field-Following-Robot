...
// Initialize to 0 speed, forward direction
...
// auto_start() polls this each iteration and exits cleanly.
...
// Drive both pins high to brake the motor (based on your original motor_stop logic)
...
void Hardware_Init(void) {
    // Set up timer 2 using course-specific register configuration
    ...
    __enable_irq();                   // CRITICAL: Enable global interrupts
}

// SysTick-based blocking delay for use within motor.c only.
// Does NOT conflict with Timer_us in receiver.c 
...
void motor_init() {
    ...
    motor_stop(); // Ensure motors start in a safe state
    Hardware_Init(); // Start the PWM background timer
}

// --- The Core PWM Interrupt Service Routine ---
// This must be named exactly TIM2_Handler to match your startup.c
...
// Left wheel goes backward, right wheel goes forward at full speed.
// Duration is set by ROTATE_180_MS – tune this on the physical robot.
...
// ================================================================
// auto_turn_left / auto_turn_right
//   Executes a 90-degree turn then stops.
//   Tune AUTO_TURN_MS on the physical robot.
//   After the turn, the field-following loop re-acquires the wire.
// ================================================================
...
{
    // No turn needed – just keep going, LEDs off
    LED_LEFT_OFF();
    LED_RIGHT_OFF();
}
...
// auto_ADC_Read
//   Minimal ADC read used inside motor.c for field following.
//   ADC must already be initialised by receiver.c before auto_start
//   is called.
...
// ================================================================
// Path table from project spec
// [intersection-1][path-1] indexed as path_table[path][intersection]
// Actions: 0=Forward, 1=Left, 2=Right, 3=Stop
// ================================================================

// CHANGED: Keep paths 1-3 as const (they never change)
...
// NEW: Separate modifiable array for custom path 4
static unsigned char custom_path_4[8] = {3, 3, 3, 3, 3, 3, 3, 3};  // Default all STOP

// NEW: Helper function to get direction at intersection
static unsigned char get_direction_at_intersection(unsigned char robot_path, unsigned char intersection_num)
{
    ...
    if (robot_path == 4) {
        ...
        // Use predefined paths (1-indexed, so subtract 1 for array)
...
// ================================================================
// auto_reset
//   Call once when entering auto mode to clear intersection state.
// ================================================================
...
// ================================================================
// auto_step
//   Called once per main loop iteration (~20ms period).
//   Reads d1/d2/d3, steers, and handles intersections.
//   All blocking operations (turns) are kept short relative to
//   the IR receive timeout so commands are never missed by more
//   than one loop cycle.
// ================================================================
               ...
               uint16_t d1, uint16_t d2, uint16_t d3)
{
    ...
    if (d3 > AUTO_THRESHOLD && !auto_in_intersection)
    {
        ...
        // CHANGED: Use helper function instead of direct array access
...
// ================================================================
// NEW: Update custom path 4 with data from receiver
// ================================================================
